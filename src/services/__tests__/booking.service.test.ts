...
describe('BookingService', () => {
  ...
  beforeEach(() => {
    ...
    mockBookingRepo = {
      ...
      bulkUpdateStatus: jest.fn(), // ✅ Add this line
    } as any;
  ...
  describe('create', () => {
    ...
    it('should throw BadRequestError if car is not available', async () => {
      ...
      require('../../repositories/car.repository').default.findById = jest
        .fn()
        .mockResolvedValue(unavailableCar);
    ...
    it('should throw ValidationError if start date is in the past', async () => {
      ...
      await expect(bookingService.create(pastData as any, mockContext)).rejects.toThrow(
        ValidationError
      );
    });
    ...
    it('should throw ValidationError if end date is before start date', async () => {
      ...
      await expect(bookingService.create(invalidData as any, mockContext)).rejects.toThrow(
        ValidationError
      );
    });
  ...
  describe('confirmBooking', () => {
    ...
    it('should throw BadRequestError if booking is not pending', async () => {
      ...
      await expect(bookingService.confirmBooking('booking-123', mockContext)).rejects.toThrow(
        BadRequestError
      );
    });

    it('should throw BusinessRuleError if deposit not paid and required', async () => {
      mockBookingRepo.findById.mockResolvedValue({
        ...mockBooking,
        depositPaid: false,
      } as any);
    ...
    it('should throw BusinessRuleError if booking expired', async () => {
      mockBookingRepo.findById.mockResolvedValue({
        ...mockBooking,
  ...
  describe('convertToContract', () => {
    ...
    beforeEach(() => {
      ...
      require('../../services/contract.service').default.create = jest
        .fn()
    ...
    it('should throw BadRequestError if booking not confirmed', async () => {
      ...
      await expect(bookingService.convertToContract('booking-123', mockContext)).rejects.toThrow(
        BadRequestError
      );
    });
  ...
  describe('expireBookings', () => {
    it('should expire multiple bookings', async () => {
      ...
      mockBookingRepo.bulkUpdateStatus.mockResolvedValue(2);
      ...
      expect(mockBookingRepo.bulkUpdateStatus).toHaveBeenCalledWith(
        ['booking-1', 'booking-2'],
        BookingStatus.EXPIRED,
        'system'
      );

      expect(mockBookingRepo.findById).toHaveBeenCalledTimes(2);
    });
    ...
    it('should handle errors gracefully', async () => {
      ...
      // ✅ Mock bulkUpdateStatus to throw error (not update)
      mockBookingRepo.bulkUpdateStatus.mockRejectedValue(new Error('Database error'));

      // Should not throw - returns error result instead
      const result = await bookingService.expireBookings();
    ...
    it('should return empty result if no expired bookings found', async () => {
      mockBookingRepo.findExpiredBookings.mockResolvedValue([]);
    ...
    it('should handle findExpiredBookings error gracefully', async () => {
      mockBookingRepo.findExpiredBookings.mockRejectedValue(new Error('Database connection error'));
  ...
  describe('getUpcomingBookings', () => {
    ...
    it('should throw ValidationError if days out of range', async () => {
      await expect(bookingService.getUpcomingBookings(0)).rejects.toThrow(ValidationError);

      await expect(bookingService.getUpcomingBookings(400)).rejects.toThrow(ValidationError);
    });
  });
});
